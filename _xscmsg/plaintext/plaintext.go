...
func (m *PlainText) Edit() []xscmsg.Field {
	if m.fields == nil {
		m.fields = []xscmsg.Field{
			...
			xscmsg.WrapRequiredField(&subjectField{xscmsg.BaseField(&m.Subject)}),
			xscmsg.WrapRequiredField(&bodyField{xscmsg.BaseField(&m.Body)}),
		}
	}
	return m.fields
}

type subjectField struct{ xscmsg.Field }
...
func (f subjectField) Help() string {
	return "This is the message subject."
}

type bodyField struct{ xscmsg.Field }
