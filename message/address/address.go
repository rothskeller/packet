...
var (
	ErrInvalidAddress     = errors.New("The address is not a valid address.")
	ErrInvalidAddressList = errors.New("The list is not a valid address list.")
)
...
// ParseList parses a string containing a list of addresses, and returns the
// parsed addresses.  The addresses may be either RFC-5322 compliant email
// addresses (non-obsolete syntax only) or packet addresses.
func ParseList(s string) (addrs []*Address, err error) {
	_, s = parseWhitespace(s)
	if s == "" {
		return nil, nil
	}
	if addr, rest, ok := parse(s); !ok {
...
// Parse parses a string containing a single address, either RFC-5322
// email address or packet address.
func Parse(s string) (*Address, error) {
	addr, rest, ok := parse(s)
	if !ok || rest != "" {
...
func parse(s string) (addr *Address, rest string, ok bool) {
