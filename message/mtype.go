...
type Type struct {
	...
	Tag string
	// HTML is the HTML filename that identifies the type of form.  It is
	// empty for messages that are not forms.
	HTML string
	// Version is the version number of the form.  It is empty for messages
	// that are not forms.
	Version string
	// FieldOrder is an ordered list of form field tags.  When provided,
	// generated forms list the fields in that order.  (Should any fields
	// be omitted from the list, they are put at the start of the form,
	// before those in the list.)  This is nil for messages that are not
	// forms.
	FieldOrder []string
	// Name is the English name of the message type, in prose case.
	Name string
	// Article is the indefinite article to use before the Name when
	// needed; it is always either "a" or "an".
	Article string
	...
	PDFBase []byte
	// create is the function to create a new outgoing message of this
	// type, with appropriate default values for fields.  It is nil if new
	// messages of this type are not supported.
	create func() Message
}

// RegisteredTypes is a map from message type tag to Type structures for
// all registered message types.  It should be treated as read-only; to
// register a new type, call Register.
