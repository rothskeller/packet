...
type extractPlainTextTest struct {
	input    string
	output   string
	notplain bool
}

var extractPlainTextTests = []extractPlainTextTest{
	{
		"From: nobody\n\nnothing\n",
		"nothing\n",
		false,
	...
	{
		actualBounceMessage,
		actualBounceMessagePlainText,
		true,
	},
}

func Test_extractPlainText(t *testing.T) {
	for i, tt := range extractPlainTextTests {
		t.Run(fmt.Sprintf("extractPlainTextTests[%d]", i), func(t *testing.T) {
			mr := textproto.NewReader(bufio.NewReader(strings.NewReader(tt.input)))
			h, _ := mr.ReadMIMEHeader()
			in, _ := io.ReadAll(mr.R)
			out, np := extractPlainText(h, in)
			if string(out) != tt.output {
				t.Errorf("extractPlainText() body=%q want %q", out, tt.output)
			}
			if np != tt.notplain {
				t.Errorf("extractPlainText() notplain=%v want %v", np, tt.notplain)
			}
		})
	}
}

const actualBounceMessage = `Received: from BY5PR09MB5379.namprd09.prod.outlook.com (2603:10b6:a03:24d::11)
...
const actualBounceMessagePlainText = `
Your message to gordon@amateur-radio.org couldn't be delivered.
amateur-radio.org suspects your message is spam and rejected it.
cert Office 365 amateur-radio.org
Sender Action Required
