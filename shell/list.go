...
func cmdList(args []string) bool {
	...
	// Read the list of symlinks.  This gives us remote message IDs.
	forEachMessageSymlink(func(local, remote string) {
		remotes[local] = remote
	})
	// Read the list of files and note which LMIs have an associated DR.
	forEachDeliveryReceipt(func(lmi string) {
		hasDR[lmi] = true
	})
	...
	forEachMessageFile(func(lmi string) {
		...
		l.item(lmi, remotes[lmi], env, hasDR[lmi])
	})
	if !l.seenOne {
