...
func sendMessage(conn *jnos.Conn, list *lister, filename string, env *envelope.Envelope, msg message.Message) bool {
	...
	if err := saveMessage(filename, env, msg); err != nil {
		fmt.Fprintf(os.Stderr, "\r\033[KERROR: %s\n", err)
		return false
	}
	removeICS309s()
...
func receiveMessage(conn *jnos.Conn, list *lister, area string, msgnum int, subjectToLMI map[string]string) (done, ok bool) {
	...
	lmi := getNextMessageID(config.MessageID)
	if msg, ok := msg.(message.IUpdate); ok {
	...
	// Create symlinks from the remote ID if we have one.
	var rmi string
	if msg, ok := msg.(message.HumanMessage); ok {
		if rmi = msg.GetOriginID(); rmi != "" {
			os.Symlink(lmi+".txt", rmi+".txt")
			if _, err = os.Stat(lmi + ".pdf"); !errors.Is(err, os.ErrNotExist) {
				os.Symlink(lmi+".pdf", rmi+".pdf")
			}
		}
	}
	removeICS309s()
...
func recordReceipt(list *lister, env *envelope.Envelope, msg message.Message, subjectToLMI map[string]string) bool {
	...
	if err = saveMessage(lmi+ext, env, msg); err != nil {
		fmt.Fprintf(os.Stderr, "\r\033[KERROR: %s\n", err)
		return false
	}
	removeICS309s()
