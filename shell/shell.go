...
func cmdDefault(args []string) (ok bool) {
	...
	case 1:
		...
		// if it hasn't.
		if env, _, err := readMessage(lmis[0]); err != nil {
			ok = false
		} else if env.IsFinal() {
			ok = cmdShow(args, "")
		} else {
...
func expandMessageID(in string, remoteOK bool) (matches []string) {
	...
	// Parse the input as either an integer or a message ID.
	if inparts = msgIDRE.FindStringSubmatch(in); inparts == nil {
		inseq, _ = strconv.Atoi(in)
	} else {
		inseq, _ = strconv.Atoi(inparts[2])
	}
	if inseq <= 0 {
		return nil
	}
	// Look for a message file with a matching LMI.
	forEachMessageFile(func(msgid string) {
		var foundparts = msgIDRE.FindStringSubmatch(msgid)
		var foundseq, _ = strconv.Atoi(foundparts[2])
		if inseq == foundseq && (inparts == nil || (inparts[1] == foundparts[1] && inparts[3] == foundparts[3])) {
			matches = append(matches, msgid)
		}
	})
	if len(matches) != 0 || !remoteOK {
		return matches
	}
	// None found.  Look for a message symlink with a matching RMI.
	forEachMessageSymlink(func(local, remote string) {
		var foundparts = msgIDRE.FindStringSubmatch(remote)
		var foundseq, _ = strconv.Atoi(foundparts[2])
		if inseq == foundseq && (inparts == nil || (inparts[1] == foundparts[1] && inparts[3] == foundparts[3])) {
