...
// Envelope is the name of the header added to a message to hold the contents of
// a message's envelope "From " line.
const Envelope = "X-Envelope-From"
// NotPlainText is the name of the header added to a message when it contains a
// (partially) non-plain-text body.
const NotPlainText = "X-Pktmsg-Not-Plain-Text"
...
// parseRawMessage parses an encoded message.  It returns an error if the
// message could not be decoded or parsed.  Even if it returns a non-nil error,
// the returned header may be non-nil and may contain incomplete message data.
func parseRawMessage(rawmsg string) (header textproto.MIMEHeader, body string, err error) {
	var (
		loc      []int
		hasenv bool
		envfrom string
	...
	if loc = fromLineRE.FindStringSubmatchIndex(rawmsg); loc != nil {
		envfrom =
		hasenv = true
		envfrom = rawmsg[loc[2]:loc[3]]
		if loc[4] >= 0 {
			// Looks like there's a timestamp on the envelope line.
			// RFC-4155 says it should be a ctime-style timestamp.
			if _, err = time.Parse(time.ANSIC, rawmsg[loc[4]:loc[5]]); err == nil {
				envdate = rawmsg[loc[4]:loc[5]]
			}
		}
		rawmsg = rawmsg[loc[1]:]
	}
	// Parse the message headers.  If we can't parse them, it's an
	// unparseable message and we go no further.  (But before we return the
	// error, we'll add the envelope information if any.)
	mm, err = mail.ReadMessage(strings.NewReader(rawmsg))
	if err == nil {
		header = textproto.MIMEHeader(mm.Header)
	} else if hasenv {
		header = make(textproto.MIMEHeader)
	}
	if hasenv {
		header.Set(EnvelopeFrom, envfrom)
		header.Set(EnvelopeDate, envdate)
	}
	if err != nil {
		return header, "", err
	}
	...
	bodyb, notplain, err = extractPlainText(header, bodyb)
	if notplain {
		header.Set(NotPlainText, "true")
	}
	if err != nil {
		return header, "", err
	}
...
func oextractPlainText(header textproto.MIMEHeader, body []byte) (nbody []byte, notplain bool, err error) {
	var (
		mediatype string
