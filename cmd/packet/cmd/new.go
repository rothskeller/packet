...
When the --copy (or -c) flag is given, the new message will be an exact copy of the named source message except for being given a new local message ID.  The source message must be an outgoing message (either sent or unsent).

When neither the --reply nor --copy flag is given, an empty message of «new-message-type» is created.  «new-message-type» must be the tag name or create key of one of the supported message types.  Use "packet forms list" to get a list of supported forms, or use "p" or "plain" to create a plain-text message.
