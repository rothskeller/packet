...
func (cio *CIO) paintBuf(n *screenBuf) {
	// If the screen buffer hasn't been used yet, initialize it.
	if cio.buf == nil {
		cio.buf = newScreenBuf(len(n.lines[0].chars))
	}
	...
	for y, nline := range n.lines {
		...
		// scroll it up.
		if y >= len(cio.buf.lines) {
			cio.buf.fill(0, len(n.lines[0].chars), y, 0)
			cio.move(0, y-1)
			cio.print(0, "\n")
		}
		oline := cio.buf.lines[y]
		...
		for x < len(nline.chars) {
			...
			cio.paintLine(startX, endX, y, nline)
		}
	...
	// buffer.
	for len(cio.buf.lines) > len(n.lines) {
		blank := screenBufLine{
			chars:  bytes.Repeat([]byte{' '}, len(cio.buf.lines[0].chars)),
			colors: make([]uint16, len(cio.buf.lines[0].chars)),
		}
		cio.paintLine(0, len(blank.chars), len(cio.buf.lines)-1, blank)
		cio.buf.lines = cio.buf.lines[:len(cio.buf.lines)-1]
	}
}

func (cio *CIO) paintLine(startX, endX, y int, line screenBufLine) {
	x := startX
	for x < endX {
		...
		cio.printb(int(color), line.chars[spanStart:x])
	}
}
