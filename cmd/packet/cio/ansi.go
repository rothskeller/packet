...
// Color constants, stored with the foreground color in the lower byte and the
// background color in the higher byte of a two-byte value.  A color constant
// with zero for either byte inherits that byte from colorNormal.
const (
...
// print prints the specifed byte in the specified color.
func (cio *CIO) printb(color int, b []byte) {
	cio.setColor(color)
	os.Stdout.Write(b)
	cio.curX += len(b)
}

// setColor sends the escape sequences needed to change the current output
// color to the one specified.
func (cio *CIO) setColor(color int) {
	if color&0xFF00 == 0 {
	...
	if color == cio.lastColor {
		return
	}
	io.WriteString(os.Stdout, "\033[")
	if color&0x00FF != cio.lastColor&0x00FF {
		fmt.Printf("38;5;%d", color&0x00FF)
	}
	if color&0x00FF != cio.lastColor&0x00FF && color&0xFF00 != cio.lastColor&0xFF00 {
		io.WriteString(os.Stdout, ";")
	}
	if color&0xFF00 != cio.lastColor&0xFF00 {
...
// clearStatus removes any status line that is being displayed.
func (cio *CIO) clearStatus() {
	if cio.haveStatus {
		io.WriteString(os.Stdout, "\r")
		cio.curX = 0
		cio.clearToEOL()
		cio.haveStatus = false
	}
}

func (cio *CIO) clearToEOL() {
