...
func (e *editor) onelineMode() (modefunc, EditResult, error) {
	...
	fieldWidth = e.valueWidth
	if fieldWidth == 0 || entryx+fieldWidth >= e.cio.Width {
		fieldWidth = e.cio.Width - entryx - 1
	}
	for {
		// Draw the label, entry area, and hint.
		buf := newScreenBuf(e.cio.Width - 1)
		buf.writeAt(0, 0, colorLabel, e.label)
		buf.fill(entryx, fieldWidth, 0, colorEntry)
		if len(e.value) <= fieldWidth && e.hint != "" &&
			entryx+fieldWidth+len(e.hint)+2 < e.cio.Width {
			buf.writeAt(entryx+fieldWidth+2, 0, colorHint, e.hint)
		}
		...
		case 0x0A, 0x0D: // Enter
			...
			if verbatim || (e.cursor != 0 && e.multiline) {
		...
		default:
			if key >= 0x20 && key <= 0x7e { // Printable character
				...
				if entryx+len(e.value) >= e.cio.Width {
