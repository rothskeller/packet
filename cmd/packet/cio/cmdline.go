...
func (cio *CIO) ReadCommand() (line string, err error) {
	if cio.InputIsTerm {
		return cio.readCommandTerminal()
	}
	return readCommandStdin()
}

func (cio *CIO) readCommandTerminal() (line string, err error) {
	var (
		cursor       int
		scroll       int
		selstart     int
	...
	for {
		var buf = newScreenBuf(cio.Width - 1)
		...
		case 0x0A, 0x0D: // Enter
			...
			cio.move(8, 0)
			cio.clearToEOL()
			cio.print(0, line) // might wrap, but that's OK
			cio.print(0, "\n")
			cio.cleanTerminal()
			return line, nil
		case 0x0B: // Ctrl-K
		...
		scroll = max(scroll, cursor-cio.Width+9)
	}
}
