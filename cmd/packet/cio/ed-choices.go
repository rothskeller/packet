...
func (e *editor) choicesMode() (modefunc, EditResult, error) {
	...
	// Draw the label.
	e.cio.print(colorLabel, setLength(e.label, e.labelWidth+2))
	entryx = e.cio.curX
	...
	for lnum, line := range choices {
		...
		for cnum, choice := range line {
			...
			e.cio.print(0, spaces[:x-e.cio.curX])
			if choice == "" {
				e.cio.print(colorEntry, spaces[:4])
			} else if e.value == choice {
				selline, selcol = lnum, cnum
				e.cio.print(colorSelected, choice)
			} else {
				e.cio.print(0, choice)
			}
		}
	}
	// Run the input loop.
	for {
		...
		if selline != 0 || selcol != 0 {
			e.cio.move(xs[selcol], selline)
			e.cio.print(0, e.value)
		}
		...
		if selline != 0 || selcol != 0 {
			e.cio.move(xs[selcol], selline)
			e.cio.print(colorSelected, e.value)
		}
	}
}
...
func (e *editor) choicesLayout(linecount int) (xs []int, choices [][]string) {
	...
	// adjust for that.
	if linecount == 1 && x+e.labelWidth+4 < e.cio.Width {
		for i := range xs {
			xs[i] += e.labelWidth + 4
		}
