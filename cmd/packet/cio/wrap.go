...
func (cio *CIO) WrapText(s string) (wrapped string) {
	for line := range strings.SplitSeq(strings.TrimRight(s, "\n"), "\n") {
		...
		for len(line) > cio.Width {
			if idx := strings.LastIndexByte(line[:cio.Width], ' '); idx > 0 {
				wrapped += line[:idx] + "\n"
				line = spaces[:indent] + strings.TrimLeft(line[idx+1:], " ")
			} else {
				wrapped += line[:cio.Width] + "\n"
				line = spaces[:indent] + line[cio.Width:]
			}
		}
		wrapped += line + "\n"
	}
