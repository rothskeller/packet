...
func emitMsgTypes(form *htmlb.Element, session *store.Session, show, focus bool, err string) {
	...
	in := row.E("div class=formInput")
	in.E("input type=checkbox id=mtype.plain name=mtype value=plain", slices.Contains(session.MessageTypes, "plain"), "checked", focus, "autofocus")
	in.E("label for=mtype.plain> Plain text message")
	in.E("br")
	var tags []string
	var tagnames = make(map[string]string)
	for mt := range message.AllTypes() {
		if emt, ok := mt.(message.EditableMType); ok {
			if tag := emt.CreateTag(); tag != "plain" && config.Get().MessageTypes[tag] != nil {
				tags = append(tags, tag)
				tagnames[tag] = emt.Name()
			}
		}
	}
	sort.Slice(tags, func(i, j int) bool {
		return capitalize(tagnames[tags[i]]) < capitalize(tagnames[tags[j]])
	})
	for _, tag := range tags {
		in.E("input type=checkbox id=mtype.%s name=mtype value=%s", tag, tag, slices.Contains(session.MessageTypes, tag), "checked")
		in.E("label for=mtype.%s> %s", tag, capitalize(tagnames[tag]))
		in.E("br")
	}
	if err != "" {
		row.E("div class=formError>%s", err)
...
func readPlainSubject(r *http.Request, session *store.Session, show bool) string {
	...
	subj := strings.TrimSpace(r.FormValue("plainSubject"))
	body := strings.TrimSpace(removeCR.Replace(r.FormValue("plainBody")))
	session.ModelMessage = "Subject: " + subj + "\n\n" + body
	if subj == "" {
		return "The subject line is required."
	}
	if s := subject.DecodePlainSubject(subj); s.SubjectSummary() == subj {
...
func readFormBody(r *http.Request, session *store.Session, show bool) string {
	...
	if msg, err := message.Parse(session.ModelMessage, "(input)"); err != nil {
		return "This is not a valid PackItForms-encoded form."
	} else if _, ok := msg.Subject().(*form.FormSubject); !ok {
		return "This is not a valid PackItForms-encoded form."
	}
	return ""
}
