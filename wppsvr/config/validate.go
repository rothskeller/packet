	...
	if c.MessageTypes == nil {
		...
		for tag, mtc := range c.MessageTypes {
			...
			case "", "IMMEDIATE", "PRIORITY", "ROUTINE":
				// nothing
			case "computed":
				if !HasComputedHandlingOrder(tag) {
		...
		for mt := range message.AllTypes() {
			if emt, ok := mt.(message.EditableMType); ok {
				if _, ok := c.MessageTypes[emt.CreateTag()]; !ok {
					if emt.CreateTag() == "plain" {
						c.MessageTypes["plain"] = new(MessageTypeConfig)
					} else {
						log.Printf("ERROR: config.messageTypes[%q] is not specified", emt.CreateTag())
						valid = false
					}
				}
