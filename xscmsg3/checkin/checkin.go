...
func create() xscmsg.Message {
	var m = checkin{Message: xscmsg.NewBaseMessage(mtype, nil)}
...
func recognize(pm pktmsg.Message) xscmsg.Message {
	...
	if !strings.HasPrefix(strings.ToLower(pktmsg.KeyValue(pm, pktmsg.FSubject)), "check-in ") {
		return nil
	}
	var m = checkin{Message: xscmsg.NewBaseMessage(mtype, pm)}
	if match := checkInRE.FindStringSubmatch(pktmsg.KeyValue(pm, pktmsg.FBody)); match != nil {
		if match[3] != "" {
...
func (m checkin) Iterate(fn func(pktmsg.Field)) {
...
func (m checkin) KeyedField(key pktmsg.FieldKey) pktmsg.KeyedField {
	switch key {
...
func (m checkin) Transmit() (to []string, subject string, body string) {
	m.encode()
	return m.Message.Transmit()
}
func (m checkin) encode() {
	if m.tacCall.Value() != "" {
		m.KeyedField(pktmsg.FSubject).SetValue(fmt.Sprintf("Check-In %s, %s", m.tacCall.Value(), m.tacName.Value()))
		m.KeyedField(pktmsg.FBody).SetValue(fmt.Sprintf("Check-In %s, %s\n%s, %s\n", m.tacCall.Value(), m.tacName.Value(), m.opCall.Value(), m.opName.Value()))
	} else {
		m.KeyedField(pktmsg.FSubject).SetValue(fmt.Sprintf("Check-In %s, %s", m.opCall.Value(), m.opName.Value()))
